...
const timingJsonPath = path.join(audioDir, 'voiceover.timing.json');
...
const main = async () => {
  ...
  let startFrame = 0;
  const out: any[] = [];
  const gen: { id: string; startFrame: number; durationInFrames: number; durationSeconds: number }[] = [];
  for (const b of beats) {
    ...
    out.push({ id: b.id, section: b.section, caption: b.text, audioSrc: `audio/${b.id}.mp3`, startFrame, durationInFrames, durationSeconds });
    gen.push({ id: b.id, startFrame, durationInFrames, durationSeconds: +durationSeconds.toFixed(3) });
    startFrame += durationInFrames;
  }

  const totalFrames = startFrame;
  writeFileSync(timingJsonPath, JSON.stringify({ provider: 'elevenlabs', modelId, voiceId, fps, totalFrames, totalSeconds: +(totalFrames / fps).toFixed(2), beats: out }, null, 2));
  writeFileSync(
    timingTsPath,
    `// AUTO-GENERATED by scripts/generate-voiceover.ts. Do not edit by hand.\n` +
      ...
      `export const generatedBeats: GeneratedBeat[] = ${JSON.stringify(gen, null, 2)};\n`,
  );
  console.log(JSON.stringify({ status: 'done', voiceId, modelId, totalSeconds: +(totalFrames / fps).toFixed(2), beats: gen.map((b) => ({ id: b.id, sec: b.durationSeconds })) }, null, 2));
};
