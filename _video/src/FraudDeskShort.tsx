...
const FollowPill: React.FC<{ frame: number; fps: number }> = ({ frame, fps }) => {
  const cta = beats.find((b) => b.section === 'cta');
  if (!cta || frame < cta.startFrame) return null;
  const pop = spring({ frame: frame - cta.startFrame, fps, config: { damping: 14, stiffness: 120 } });
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: '#efe7d6' }}>
      <AbsoluteFill style={{ opacity: fadeIn }}>
        <OffthreadVideo
          src={staticFile('capture/pilot.mp4')}
